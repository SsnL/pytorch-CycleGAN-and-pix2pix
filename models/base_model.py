...
class BaseModel():
    ...
    def save_variable(self, var, var_label, epoch_label, gpu_ids):
        save_filename = '%s_var_%s.pth' % (epoch_label, var_label)
        save_path = os.path.join(self.save_dir, save_filename)
        torch.save(var.cpu(), save_path)
        # if len(gpu_ids) and torch.cuda.is_available():
        #     network.cuda(device_id=gpu_ids[0])

    # helper loading function that can be used by subclasses
    def load_variable(self, var_label, epoch_label):
        save_filename = '%s_var_%s.pth' % (epoch_label, var_label)
        save_path = os.path.join(self.save_dir, save_filename)
