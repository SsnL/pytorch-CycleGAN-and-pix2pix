...
class MultiCycleGANWithHubModel(BaseModel):
    ...
    def backward_D(self, label):
        ...
        new_fake = torch.cat(tuple(self.fakes[(label, to_label)] for to_label in self.inputs if to_label != label), 0)
    ...
    def backward_G(self, label):
        ...
        total_loss = loss_G + loss_cycle * self.lambdas[label]
        total_loss.backward()
        ...
        for label in self.inputs:
            ...
            for to_label in self.inputs:
                if label == to_label:
                    continue
                # fake images
                visuals['fake_' + label + '_' + to_label] = util.tensor2im(self.fakes[(label, to_label)].data)
                # rec images
                visuals['rec_' + label + '_' + to_label] = util.tensor2im(self.recs[(label, to_label)].data)
                # identity rec images
                if self.opt.identity > 0.0:
                    raise NotImplemented
                    for label in self.inputs:
                        visuals['idt_' + label] = sutil.tensor2im(self.idt_recs[(label, to_label)].data)
