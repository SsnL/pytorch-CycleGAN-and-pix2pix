...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain or self.estimate_weight:
    ...
    def get_current_visuals_at(self, i):
        ...
        if i < self.real_A.size(0):
            suff_A = '(%.4f)' % self.loss_weights_A[i] if self.estimate_weight else ''
        ...
        if i < self.real_B.size(0):
            suff_B = '(%.4f)' % self.loss_weights_B[i] if self.estimate_weight else ''
